...
const MultiSpanHighlight = ({original, highlightSpans, highlightStyles}) => {
    if (original && highlightSpans && highlightStyles) {
        ...
        highlightSpans.forEach((s, sIndex) => {
            ...
            if (s[1] > curIndex) {
                spanList.push(<span key={`${curIndex}_${s[1]}`}
                                    className={highlightStyles[sIndex]}>{original.slice(curIndex, s[1])}</span>);
                curIndex = s[1];
            }
        });
...
const MySaliencyMaps = ({interpretData, questionTokens, passageTokens, interpretModel, requestData}) => {
    ...
    return <SaliencyMaps interpretData={allInterpretData} inputTokens={inputTokens} inputHeaders={inputHeaders}
                         interpretModel={interpretModel} requestData={requestData}/>
}
...
const Attacks = ({attackData, attackModel, requestData}) => {
    ...
    const inputReduction = model && model.toLowerCase().includes('naqanet') ?
        ...
        : (
            <InputReductionPanel>
                <InputReductionComponent reducedInput={reducedInput}
                                         reduceFunction={attackModel(requestData, INPUT_REDUCTION_ATTACKER, NAME_OF_INPUT_TO_ATTACK, NAME_OF_GRAD_INPUT)}/>
            </InputReductionPanel>
        )
    ...
    return (
        <OutputField label="Model Attacks">
            <Collapse>
                {inputReduction}
                <HotflipPanel>
                    <HotflipComponent hotflipData={hotflipData}
                                      hotflipFunction={attackModel(requestData, HOTFLIP_ATTACKER, NAME_OF_INPUT_TO_ATTACK, NAME_OF_GRAD_INPUT)}/>
                </HotflipPanel>
            </Collapse>
        </OutputField>
    )
                ...
                if (question && passage && spans && value) {
                    return (
                        <section>
                            ...
                            <MySaliencyMaps interpretData={interpretData} questionTokens={questionTokens}
                                            passageTokens={passageTokens} interpretModel={interpretModel}
                ...
                if (question && passage && numbers && value !== undefined) {
                    return (
                        <section>
                            ...
                            <OutputField label="Explanation">
                                The model used the arithmetic expression <ArithmeticEquation
